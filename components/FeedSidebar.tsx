  ...
  return (
    <aside className="hidden lg:block w-[368px] flex-shrink-0 pl-10 pr-6 border-l border-divider pt-10">
      <div className="sticky top-0 space-y-10 pb-10">
        {/* Staff Picks */}
        <section>
          <h4 className="text-base font-semibold mb-6">Staff Picks</h4>
          <div className="space-y-5 mb-4">
            {staffPicks.map((story) => (
              <Link href={`/story/${story.id}`} key={story.id} className="block">
                ...
                <p className="text-xs text-neutral-500 mt-3">{story.readTime} min read</p>
              </Link>
            ))}
          </div>

          <Link href="/story" className="text-neutral-500 text-sm hover:underline">See the full list</Link>
        </section>
        ...
        <section>
          ...
          <Link href="/story" className="text-gray-500 text-sm hover:underline">See more topics</Link>  
        </section>
        ...
        <section>
            ...
            {suggestedAuthors.map((author) => (
              <div key={author.id} className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neutral-500 line-clamp-2 mt-1 w-[80%]">{author.bio}</p>
                </div>
          ...
          <Link href="/story" className="text-gray-500 text-sm hover:underline">See more suggestions</Link>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {trendingTags.map((tag) => (
              <Link key={tag} href={`/tag/${tag.toLowerCase()}`} className="rounded-full bg-neutral-100 px-3 py-1.5 text-xs text-neutral-500 hover:bg-secondary transition-colors">
                {tag}
              </Link>
            ))}
          </div>
        ...
        {/* Footer links */}
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-neutral-500 pt-4">
